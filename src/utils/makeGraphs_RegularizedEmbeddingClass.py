...
def printLearningCurves(allLoss, allLossMSE, allPSNR, allSSIM, curvesPath=None): 
    colors = {'Loss': 'darkcyan', 'Metrics':'crimson'}
    fig, ax = plt.subplots(3, figsize=(10,10), dpi=300, sharex=True)
    # First Line (Loss)
    ax[0].set_ylabel('MSE Loss')
    ax[0].plot(range(1,len(allLoss['train'])+1), allLoss['train'], color='gold', label='Training MSE+VQVAE', linewidth=1.5)
    ax[0].plot(range(1,len(allLoss['val'])+1), allLoss['val'], color='gold', linestyle='dashed', label='Validation MSE+VQVAE', linewidth=1.5)

    ax[0].plot(range(1,len(allLossMSE['train'])+1), allLossMSE['train'], color=colors['Loss'], label='Training MSE', linewidth=1.5)
    ax[0].plot(range(1,len(allLossMSE['val'])+1), allLossMSE['val'], color=colors['Loss'], linestyle='dashed', label='Validation MSE', linewidth=1.5)
    ax[0].set_yscale('log')
...
# ---------------------------------------------------------------------------------------
# Show individual predictions and anomaly maps (AM) (from Super Class) 
# ---------------------------------------------------------------------------------------
...
def printPredAndAM_singleFile(input, prediction, filePath, Vmin=0, Vmax=255):

    if prediction.shape[-1] == 1: 
        prediction = np.squeeze(prediction, axis=-1)
        input      = np.squeeze(input, axis=-1)
    
    vmin, vmax = min(np.min(input), np.min(prediction)), max(np.max(input), np.max(prediction))
    fig, ax = plt.subplots(1, figsize=(8,8), dpi=300)
    ax.imshow(input, cmap = 'gray', interpolation = 'bicubic', vmin=vmin, vmax=vmax)
    ax.axis('off')
    fig.tight_layout()
    plt.savefig(filePath + '_INPUT.eps')
    plt.close()

    fig, ax = plt.subplots(1, figsize=(8,8), dpi=300)
    ax.imshow(prediction, cmap = 'gray', interpolation = 'bicubic', vmin=vmin, vmax=vmax)
    ax.axis('off')
    fig.tight_layout()
    plt.savefig(filePath + '_REC.eps')
    plt.close()

    fig, ax = plt.subplots(1, figsize=(8,8), dpi=300)
    diff_im = diff(input, prediction)
    am = ax.imshow(diff_im, cmap = 'cividis', interpolation = 'bicubic',alpha=.9)
    ax.axis('off')
    fig.tight_layout()
    plt.savefig(filePath + '_DIFF.eps')
    plt.close()

    fig, ax = plt.subplots(1, figsize=(8,8), dpi=300)
    cbar = fig.colorbar(am, ax=ax)
    for t in cbar.ax.get_yticklabels():
        t.set_fontsize(30)
    ax.remove()

    fig.tight_layout()
    plt.savefig(filePath + '_COLORBAR.eps',bbox_inches='tight')
    plt.close()
